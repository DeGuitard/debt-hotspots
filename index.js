...
let computeScore = function (node) {
  return new Promise((resolve, reject) => {
    exec("wc -l < " + node.path, function (error, lineCount) {
      exec(
        `git -C ${path} rev-list --count HEAD ${node.path}`,
        function (error, commitCount) {
...
let sumScores = function (obj) {
  if (obj.type === "file" || !obj.children) return obj.score;
  else obj.score = 0;
...
let tree = dirTree(path, { exclude: ignoreList }, (node) =>
  operations.push(computeScore(node))
);
